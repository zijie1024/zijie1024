...
const formatMonthYear=value=>new Intl.DateTimeFormat("en",{
  month:"short",
  year:"numeric",
  timeZone:"UTC"
...
const dayDiff=(a,b)=>Math.round(
  (parseDate(a)-parseDate(b))/86400000
);
...
function render(themeName){
  ...
  out.push(`<defs><style>`);
  out.push(
    `.t{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,`+
  ...
  // Command outside the card
  out.push(
    `<text x="12" y="20" class="t p">`+
    `${escapeXml(USERNAME)}@github:~$`+
    `</text>`
  );

  out.push(
    `<text x="190" y="20" class="t c">`+
  ...
  out.push(
    ...
    `rx="13" fill="${theme.background}" stroke="${theme.border}"/>`
  );

  // Summary
  out.push(
    `<text x="28" y="66" class="t s">`+
    `${calendar.totalContributions} contributions in the last year`+
    `</text>`
  );

  out.push(
    `<text x="1170" y="66" text-anchor="end" class="t m">`+
    `${escapeXml(period)}`+
    `</text>`
  );
  ...
  for(const month of calendar.months){
    ...
    if(
      ...
      usedMonthColumns.has(weekIndex)
    ){
      continue;
    }
    ...
    out.push(
      `<text x="${gridX+weekIndex*step}" y="87" class="t m">`+
  ...
  for(const [label,row] of weekdayLabels){
    out.push(
      `<text x="28" y="${gridY+row*step+10}" class="t m">`+
      `${label}`+
      `</text>`
    );
  }
  ...
  weeks.forEach((week,weekIndex)=>{
    week.contributionDays.forEach(day=>{
      ...
      out.push(
        ...
        `rx="2" fill="${color}">`+
        `<title>${escapeXml(day.date)}: ${countText}</title>`+
        `</rect>`
      );
    });
  });
  ...
  // Legend
  const legendY=205;
  const legendStart=988;

  out.push(
    `<text x="${legendStart}" y="${legendY}" class="t m">Less</text>`
  );
  ...
  levels.forEach((level,index)=>{
    out.push(
      `<rect x="${legendStart+31+index*17}" `+
      `y="${legendY-10}" width="12" height="12" rx="2" `+
      `fill="${theme.levels[level]}"/>`
    );
  });

  out.push(
    `<text x="${legendStart+120}" y="${legendY}" class="t m">More</text>`
  );
